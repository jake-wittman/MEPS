...
The data we used comes from the full-year consolidated file from MEPS for the 12 year time span. Our analysis is adjusted for the complex survey design
...
age_adjusted_preventive %>% 
  ...
  group_by(stratifier, strata) %>%
  select(stratifier, label, `2008`:`2020`) %>%
  gt() %>% 
  fmt_number(columns = `2008`:`2020`, decimals = 1) %>% 
