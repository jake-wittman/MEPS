...
cleaned_preventive_df <- left_join(clean_age_adjusted, cleaned_jp_apc, by = c('stratifier', 'strata')) %>% 
  ...
  mutate(strata = case_when(as.character(strata) == 'Poor/Negative' ~ 'Income < 100% of the poverty line',
                            as.character(strata) == 'Low income' ~ 'Income 100% - 199% of the poverty line',
                            as.character(strata) == 'Middle income' ~ 'Income 200% - 399% of the poverty line',
                            as.character(strata) == 'High income' ~ 'Income > 400% of the poverty line',
