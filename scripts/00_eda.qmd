...
diab_svy <- diab_list[[13]] %>%
  as_survey_design(ids = VARPSU_all, weights = PERWT, strata = VARSTR_all, nest = TRUE)
...
test_table <- diab_dat %>% 
  ...
  filter(FLU_all %in% c('Within last year')) %>% 
  mutate(FLU_all = fct_drop(FLU_all),
         FLU_all = ifelse(FLU_all == 'Within last year', 'Flu vaccine within last year', FLU_all)) %>% 
  as_survey_design(ids = VARPSU_all, weights = DIABW, strata = VARSTR_all, nest = TRUE) %>% 
  select(FLU_all,
         Race = RACE_all,
         Education = HIDEG_all,
  ...
  tbl_strata(
    ...
    ~.x %>%  
      select(-year) %>% 
      tbl_svysummary(by = FLU_all,
                     statistic = list(all_categorical() ~ '{p}%'))
  ) %>% 
  modify_header(label = "**Flu vaccine within last year**",
                all_stat_cols() ~ "")

test_table2 <- diab_dat %>% 
  #filter(year %in% 2008:2011) %>% 
  filter(DSA1C53 %in% c('2 or more A1C tests in a year')) %>% 
  mutate(DSA1C53 = fct_drop(DSA1C53)) %>% 
  as_survey_design(ids = VARPSU_all, weights = DIABW, strata = VARSTR_all, nest = TRUE) %>% 
  select(DSA1C53,
         Race = RACE_all,
         Education = HIDEG_all,
  ...
  tbl_strata(
    ...
    ~.x %>%  
      select(-year) %>% 
      tbl_svysummary(by = DSA1C53,
                     statistic = list(all_categorical() ~ '{p}%'))
  ) %>% 
  modify_header(label = "**2 or more A1C tests in a year**",
                all_stat_cols() ~ "")
...
```{r}
tar_load(contains('focus'))
dats <- list(overall = overall_focus, edu = edu_focus, race = race_focus, sex = sex_focus, age = age_focus)
test <- bind_rows(dats, .id = 'stratifying_variable')
test2 <- test %>%
  filter(variable != 'DENT_all') %>% 
  mutate(strata = as.character(coalesce(HIDEG_all, RACE_all, SEX, AGE_all))) %>%
  select(-HIDEG_all,-RACE_all,-SEX,-AGE_all) %>%
  mutate(strata = case_when(is.na(strata) ~ 'Overall',
                            TRUE ~ strata))
...
test3 <-  test2 %>%
  ...
  group_split(stratifying_variable) 
plots <- test3 %>% 
   map(
    ~ ggplot(.x, aes(
      x = year,
...
plots[[1]] <- plots[[1]] + labs(color = 'Age (grouped)')
plots[[2]] <- plots[[2]] + labs(color = 'Highest degree \nobtained')
plots[[3]] <- plots[[3]] + labs(color = "")
plots[[4]] <- plots[[4]] + labs(color = 'Race/Ethnicity')
plots[[5]] <- plots[[5]] + labs(color = 'Gender')
plots <- list(
  plots[[3]],
  plots[[1]],
  plots[[2]],
